...
import { readFile, readFileSync, writeFileSync, writeFile } from 'fs';
...
app.get('/:name', (req, res) => {
    ...
    readFile(path + name + '.json', 'utf8', (err, data) => {
        if (err) {
            console.log(err);
...
app.post('/:titulo/:id', (req, res) => {
    ...
    const data = readFileSync(path+ name+'.json');
    ...
    writeFileSync(path+name+'.json', JSON.stringify(jsonData, null, 2));
...
app.delete('/:titulo/:id/:code', (req, res) => {
    ...
    writeFile(path+name+'.json', JSON.stringify(removeJson, null, 2), err => {
    res.status(204).send();
    });
});
